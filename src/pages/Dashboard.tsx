...
export default function Dashboard() {
  ...
  const weeklyTotalDur = Math.max(1, Math.floor((weeklyResetAt - weeklySetAt) / 1000))
  ...
  return (
    <main className="px-margin-mobile max-w-2xl mx-auto space-y-gutter py-6 lg:py-8">
      <HeroCountdown secondsLeft={secsLeft5h} totalDuration={totalDur5h} />
      <section className="grid grid-cols-1 md:grid-cols-2 gap-gutter">
        <FiveHourCard secondsLeft={secsLeft5h} totalDuration={totalDur5h} onSet={handleSet5h} />
        <WeeklyCard
          secondsLeft={weeklySecsLeft}
          totalDuration={weeklyTotalDur}
